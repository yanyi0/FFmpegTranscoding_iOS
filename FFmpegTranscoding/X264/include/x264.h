...
#define MAX_ROI_REGION_NUM  6  //0~6
...
typedef struct x264_param_t
{
	...
	/* ROI Properties */
#ifdef ROI
	int         i_regionCount; //0~6
	int         i_level;       //0~3
	int         preROI;
	int         curROI;
	int         countROI;
	SRegionConfig  sRegion[MAX_ROI_REGION_NUM];
#endif

#ifdef HIGHQUALITY
    float        complex_rateI;        
    float        complex_rateP; 
	int          complex_ratPP; //no
	float        complex_ratePA; //dogs_1280x720
	//int          complex_rateAII;//VideoDance2_1280x720
	//int          complex_rateAPI;//VideoDance2_1280x720
	int          complex_allDQP;
	double       buffer_fill;
	int          flag_I;//IDR
	float        average_Iq;
